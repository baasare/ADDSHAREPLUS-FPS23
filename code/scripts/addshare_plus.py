...
class AdditiveClient:
    ...
    def end_training(self, threshold):
        pd.DataFrame.from_dict(self.record).to_csv(
            f"../../results/numerical/additive_random_{threshold}/{self.dataset}/client_{self.client_id}.csv",
            index=False, header=True)


def process_end_training(client, threshold):
    client.end_training(threshold)


if __name__ == "__main__":
    print(f"EXPERIMENT: ADDSHARE PLUS")
    for threshold in [0.05, 0.15, 0.25, 0.35, 0.45]:
        for dataset in DATASET:
            print(f"DATASET: {dataset.upper()}")
            indexes = fetch_index(dataset)
            (x_train, y_train), (x_test, y_test) = fetch_dataset(dataset)

            fl_server = Server(NODES, f'additive_random_{threshold}', dataset)
            ...
            for _ in range(ROUNDS):
                weights_buffer = []
                fl_update = fl_server.start_round(weights_threshold=threshold)
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                futures = [executor.submit(process_end_training, client, threshold) for client in clients]
