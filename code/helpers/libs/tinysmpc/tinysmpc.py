...
from .secret_sharing import n_from_shares, n_to_shares, n_to_tensor_shares, n_from_tensor_shares
...
class PrivateScalar():
    ...
    def share_tensor(self, machines, Q=None):
        '''Split self.value into secret shares and distribute them across machines (tracked in a SharedScalar).'''
        shares = n_to_tensor_shares(self.value, machines, Q)
        return SharedScalar(shares, Q)
...
class SharedScalar():
    ...
    def reconstruct_tensor(self, owner):
        '''Send all shares to one machine, and reconstruct the hidden value as a PrivateScalar.'''
        value = n_from_tensor_shares(self.shares, owner, self.Q)
        return PrivateScalar(value, owner)
